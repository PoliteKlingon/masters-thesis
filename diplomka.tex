...
The simplest yet most restrictive approach is for Bob to lock preemptively the instances that he intends to interact with while offline, effectively mitigating the risk of any conflict. 

An instance-granular locking mechanism must be implemented, and Bob, before going offline, has to be able to mark the instances that he needs in offline and lock them. Optionally, instances might be locked together with all their ``subinstances'' --- that is, instances that have a $n:1$ relation with said instance.
