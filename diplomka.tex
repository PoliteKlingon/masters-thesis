...
\thesissetup{
    ...
    abstract    = {%
      This thesis documents the process of designing and implementing an enterprise mobile application with offline support in a real-world scenario with specific functional requirements to be fulfilled. By analyzing\hyphenation{a-na-ly-zing} viable solutions and architectures, this thesis also serves as a reference for architects to be consulted when designing an optimal approach for a similar application.
    },
    thanks      = {%
...
During the thesis-making process, I used the ChatGPT \gls{llm} developed by OpenAI \footnote{https://chatgpt.com/} for searching for relevant literature and other sources. It also helped me generate the \texttt{.bib} bibliography entries for citations in the thesis afterwards. Finally, I used it to help me find appropriate word translations and synonyms. I take full responsibility for the aforementioned generated content.
...
The thesis consists of 4 chapters; the first chapter investigates the concept of offline-enabled mobile applications and their specific challenges and architectural demands. The second chapter explores the approaches to be taken when designing an architecture and technology\hyphenation{tech-no-lo-gy} stack of such application --- each approach is further documented, stating its strengths and weaknesses and practical scenarios in which each of them play their prime. Here the thesis not only focuses on different frameworks or languages, but also, the challenges introduced in the first chapter are confronted with different technical solutions and those are evaluated.

The third and fourth chapters focus on selecting the right approaches in different areas based on the functional and non-functional requirements, implementing a mobile frontend client, unique issues, and other aspects of the practical execution of the project in the context of a real solution for a real customer. 

~

Mobile applications are often a desirable part of a software solution. In the modern world where everyone uses their smartphone for a variety of tasks, it is expected from software companies to provide support for these devices in enterprise solutions. As there are areas without internet coverage, it is often expected that the applications will have some degree of offline independence. Offline-first (sometimes also called local-first) applications are becoming more and more relevant with rising user expectations for availability and maturing ecosystems that can support it \cite{OfflineFirst}. 

Incorporating the offline functionality to an application brings certain technical, but also conceptual challenges --- raising questions like what is the source of truth, how should synchronization conflicts be resolved, and much more.

\chapter{Offline-enabled mobile applications}
First, it is needed to take a look at offline-enabled enterprise mobile applications as a whole and identify the new challenges they have to solve compared to conventional mobile applications. Offline-enabled means an application that provides a significant part of its functionality without access to a server; while the extent of said functionality may vary based on a particular project and customer's needs. 
...
Therefore, the local data should be periodically or manually updated to minimize the occurrence of synchronization conflicts. The reason why the system should support partial changes management is that if there is none, the only option of a local data update is to bulk-download all data, as if there were none stored, and overwrite any stored data snapshot.
...
Synchronizing the changes made on the client to the server and managing\hyphenation{ma-na-ging} potential conflicts is arguably one of the most difficult tasks to tackle in the field of offline-enabled applications. The task is to synchronize all changes made in offline mode to the server when coming back online as seamlessly as possible. 

The reason why conflicts arise is that the system, and therefore (generally) any data in it, can be accessed and mutated by multiple users, while one or more users may be making their changes in offline mode. There are two general approaches:
...
\begin{enumerate}
  ...
  \item Business logic: it may not be possible to implement the state-based approach in some cases due to functional requirements --- when a change of an attribute has its meaning. Imagine a package delivery app where the package has a state (in warehouse, in delivery, delivered). The courier's phone is currently not connected to the interet as he picks up the package from a warehouse and delivers it to the customer. It is only after that when their phone finally connects to the internet. In state-based approach, the package would go straight from $inWarehouse$ state to $delivered$ state --- which is probably not the expected behaviour.
\end{enumerate}
  
\subsection{Offline authentication, authorization}
Application usability strongly relies on the user remaining authenticated while offline --- they cannot be automatically signed out when their token expires. Either the token-checking logic has to be suspended while offline, or the server response to such request must be mocked locally as a succesful call.
...
Yet there is still an additional issue. Even when the user is signed out from the application, the local data is still present in the storage of the device. In case this poses a considerable security issue in the customer's context, the data must be encrypted by the application or automatically deleted after a certain amount of time.
...
But there has been a strong tendency to favor browser-based solutions over native ones in recent history, as \citefullauthor{WebAppsReplacingNative} \cite{WebAppsReplacingNative} comments. He argues that Google has been one of the biggest driving forces on the market and that the sheer market share of their browser, Google Chrome, can in part be attributed to this transformation. The architecture in question is \gls{spa}, specifically in form of a \gls{pwa}. 
...
On the other hand, there certainly are disadvantages to this approach. The greatest one being the fact that when multiple operating systems need to be supported, there have to be multiple codebases in use --- one for each platform. 
...
\begin{enumerate}
  ...
  \item Axios HTTP\footnote{https://axios-http.com/} is a promise-based \gls{http} client with concise syntax and powerful tools like interceptors or proxies.
  ...
  \item Zod\footnote{https://zod.dev/} is a TypeScript-first validation library that enables building complex validation schemas with the option to infer types from the schemas to use. It is fully compatible with React Hook Form.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth * 10 / 31]{images/download1.png}
    \includegraphics[width=\textwidth * 10 / 31]{images/download2.png}
    \includegraphics[width=\textwidth * 10 / 31]{images/download3.png}
    \caption{Data download dialog}
    \label{fig:download}
\end{figure}
...
\begin{enumerate}
  \item It queries the browser whether it is online.
  \item If so, it just passes the request.
  \item If not, it enqueues the request into the request \gls{fifo} and aborts the original request. For this purpose, in the response interceptor, there is a filter for aborted responses --- they are not to be propagated into the app as they have been aborted by the request interceptor.
\end{enumerate} 

The request \gls{fifo} itself is persisted in the browser's local storage in order to survive app reloads\footnote{Its code is in \texttt{src/offline/request-fifo.ts}}. It stores the so-called \textit{Axios configs} --- objects storing all the relevant attributes, data, metadata, and options for a request, like \gls{url}, method, request body, parameters, headers, flags, transformers or proxies \cite{axios_req_config}. Later, when online, the config can just be passed to Axios, which composes the request and transmits it.

\subsection{Incorporating the staged data}
When offline, the application fetches data as normal (they are, in fact, served from cache by the service worker), then retrieves staged data from the \gls{fifo}, and finally decides how to merge the fetched data with the staged data --- overlaying them, proritizing one over another or any other merging strategy. As an example, let's take the Activity detail. The Activity is fetched from \texttt{/api/activity/<activityId>}, its staged data retrieved by \texttt{getStagedActivityData} from the queue, and the component that renders it, in this case, prioritizes the staged data over the fetched data.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth * 10 / 31]{images/offline1.png}
    \includegraphics[width=\textwidth * 10 / 31]{images/offline2.png}
    \includegraphics[width=\textwidth * 10 / 31]{images/offline3.png}
    \caption{Synchronizing dialog}
    \label{fig:offline}
\end{figure}
...
The second approach, the one which was chosen and implemented in this project, is to maintain a second request pool, this time particularly\hyphenation{par-ti-cu-lar-ly} for attachment requests. This does not need to be a queue as attachment adding is not order-dependent. It is stored as an array in the indexedDB storage space as it may be quite considerable in size. The application stores these requests separately and with the file content directly as the body --- the file server request is not stored nor created. The requests are abortable (deletable from the synchronizing dialog). 
...
The result is a comprehensive study on offline-enabled enterprise applications and their architectures, accompanied by an implemented application that demonstrates the discussed concepts in practice. 
